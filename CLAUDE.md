...
- `Presentation\Command\DispatchChecksCommand` (`app:dispatch-checks`) calls `Infrastructure\Scheduler\DispatchDueChecksService::dispatchDueChecks()`, the single source of truth for "which monitors are due", which dispatches `UptimeCheckMessage`/`SmtpCheckMessage` for them onto the `async` transport.
...
- Nothing dispatches automatically without a running worker: `messenger:consume async` must be running for the check/analytics messages to actually process, and `app:dispatch-checks` must run every minute to produce them in the first place.
- **In production**, all three (`app:dispatch-checks`, `messenger:consume async`, `app:poll-imap`) are kept alive without a real process supervisor via `zenstruck/schedule-bundle`: `src/Infrastructure/Scheduler/AppScheduleBuilder.php` (auto-registered — implementing `ScheduleBuilder` is enough, no `services.yaml` entry) declares each as a bounded, `withoutOverlapping()`-guarded task (`app:dispatch-checks`, `messenger:consume async --time-limit=55`, `app:poll-imap`), each `everyMinute()`. All run as real OS subprocesses (`Schedule::addProcess()`, not `addCommand()`) rather than in-process — zenstruck's in-process command runner shares one `Application`/container across every due task in a single `schedule:run` invocation, so a fatal Doctrine error in one command (closing the EntityManager) would otherwise take the others down with it; separate processes keep them isolated. The server needs exactly **one** cron entry — `* * * * * php bin/console schedule:run --env=prod` — instead of one raw crontab line per task; `schedule:run` figures out what's due and runs it. `symfony/lock` (installed alongside, `LOCK_DSN=flock` by default — fine for a single-server deploy) backs `withoutOverlapping()`.
- **`app:dispatch-checks` is invoked as a direct scheduled command, not via symfony/scheduler's `RecurringMessage`/`#[AsSchedule]`** (an earlier `App\Schedule` class did this, dispatching a `DispatchChecksMessage` onto an auto-registered `scheduler_default` Messenger transport whose handler called the same service) — that mechanism silently stopped producing due messages in production (monitors stuck "Pending", `messenger_messages` staying empty, `messenger:consume scheduler_default` reporting nothing to do, with no error to diagnose from). It was removed entirely in favor of calling the command straight from `AppScheduleBuilder`, which is simpler to verify (`schedule:list --detail`, direct manual invocation) on shared hosting.
